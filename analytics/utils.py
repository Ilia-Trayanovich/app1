...
def get_graph_data():
    ...
    # Получение всех данных, исключая None
    all_prices = Flats.objects.exclude(price=None).values_list('price', flat=True)
    all_sizes = Flats.objects.exclude(apartment_size=None).values_list('apartment_size', flat=True)
    all_floors = Flats.objects.exclude(floor=None).values_list('floor', flat=True)  # Исключаем None

    # Проверка длин всех данных
    ...
    graph_data = {
        ...
        'floors': [floor for floor in all_floors if floor is not None],  # Удаление None
    }

    # Обрезка данных до минимальной длины, чтобы они были одинаковыми
    min_length = min(len(graph_data['sizes']), len(graph_data['prices']), len(graph_data['floors']))
